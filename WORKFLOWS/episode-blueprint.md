...
revision_note: "2026-09-11 — PROVISIONAL banner added (dec-036): the short-form axis is want-first, so `## Flaw` reads as an OUTPUT of the escalations rather than the input they are angled against. Banner only — no step, field, script or budget changed, so no re-pack is owed for this edit. The full flaw-first → want-first rework is deferred by CRE behind a state trigger (the premise floor is producing in-band pieces), tracked at `^backlog-blueprint-harden-want-first`. Novel route untouched. Prior: 2026-09-05 — Inputs name the ratified shape.md (episode-harden, S1.75) as run 2's structural source; CRE widened the 'nowhere else' clause the same day the harden intent was ratified. Precedence on run 2: premise wins on overlap, shape next, arc chain + DEV additive (DEV for the aesthetic layer). Mirrored in skills-src/episode-blueprint/SKILL.md Step 1; re-pack owed (DIR-009). Prior: 2026-09-04 (route v4) — runs TWICE: run 1 at S1.5 as before, run 2 at S3 as a MANDATORY regeneration after the new S2 dream-catching leg and episode-feedback move the premise (CRE: a stale blueprint reaching the runway is the failure case). Run 2 may read WRITING/SHORTS/DEV/ as an additive escalation source; premise still wins on overlap; draft.md stays excluded on both runs. Run 1's [ANGLE MISSING — CRE] hand-backs now set the S2 agenda. DIR-017's divergence clause clarified as governing the mic, not the planning stage — the plan may be tight. Re-pack owed (DIR-009). Prior note: CRE ruled the deepened triage's `## Arc chain` in as a fourth escalation source (Inputs, Step 2, guards; mirrored in skills-src/episode-blueprint/SKILL.md Step 1). Closes the S0.5 to S1.5 seam premise-forge v2 opened upstream. The plan-only rule is unchanged: draft.md stays excluded. Precedence: premise wins on overlap, the arc chain is additive where the premise is silent. Re-packed + installed 2026-09-04; installed body verified to carry the fourth source."
---

# episode-blueprint

> **SUPERSEDED FOR WIW — 2026-09-12.** Both runs now live in [[WORKFLOWS/wiw-story-engine]] **Step 3 Escalations** (run 1 at S1.75, the S3 regeneration as a re-run carrying `## Your notes` verbatim), writing `spine.md` (which replaces `blueprint.md` + `shape.md`). Escalations are trigger → Therefore → But (naming the denied assumption) → Penalty (naming the subtraction), the Climax is the ultimate Therefore, the Cut and Pay-off are named, and budgets sum inside the band rather than to a center; no flaw, no angle-on-flaw, no variance line. This doc's 2026-09-11 PROVISIONAL banner is discharged. Doc and install untouched pending a live run of the engine; do not blueprint a new WIW episode from this doc. The novel route is not affected (it never used this tool). Provenance: [[WORKFLOWS/intents/wiw-story-engine]].
...
S1.75 harden                episode-harden → shape.md, ratified (2026-09-05) — run 2's structural source
S2   dream-catching         brainstorm → dev-capture into WRITING/SHORTS/DEV/ (route v4)
S3   synthesis + REGEN      episode-feedback, then ← this workflow again, MANDATORY (route v4) — reads shape.md
...
> **PROVISIONAL — the short-form axis is want-first (dec-036, CRE-ruled 2026-09-11).** This doc's spine is flaw-first: `## Flaw` is an input the escalations are angled against. dec-036's container clause runs the other way for short form — a 2,500-word piece cannot pay for the lead-up that establishing a flaw requires, so the piece opens on a **want the character has no agency to satisfy**, with trait and want legible in the opening move, and the **flaw is discovered in the response to the blockage** rather than established before it. Read `## Flaw` as an output of the escalations until this doc is reworked. **The rework is deferred by CRE**, trigger: *the premise floor is producing in-band pieces* — not date-bound. The novel route keeps flaw-first and is untouched.
...
- **CRE's articulated escalations** — from the premise, the rulings block, **the deepened triage's `## Arc chain`**, **the ratified `shape.md` (run 2)**, or the session. Nowhere else. **`draft.md` is never read** (plan-only, CRE-ruled 2026-09-03): a brought piece whose escalations live only in its prose has them stated by CRE in the run.
- **`shape.md` — run 2's structural source (CRE widened the "nowhere else" clause 2026-09-05).** Written at S1.75 by [[WORKFLOWS/episode-harden]] and carried through the S2 brainstorm: the want, the MoT question and answer scope, the ending, each rung with its resistance line, the flaw, and the ratified cast and setting sheet. **Precedence on run 2:** the **premise wins on overlap** (episode-feedback has just amended it) → **`shape.md` next** → the arc chain and the S2 DEV material **additive**, DEV for the aesthetic layer specifically. Read only entries tagged `[ruled]` or `[recommended → ratified]` as committed; a `[NOT NAMED — CRE]` entry is a hand-back exactly as an arc-chain one is; a bare `[recommended]` on a `status: draft` shape is **not** a ruling. Absent on run 2 → record the absence in `sources_read` and proceed on the prior sources (harden is a step, not a gate). Never read on run 1 — it does not exist yet.
...
2. **Locate + gather** — premise, rulings block, triage (**including its `## Arc chain` when deepened**; premise wins on overlap, the chain is additive where the premise is silent), **on run 2 the ratified `shape.md`** (premise → shape → chain/DEV, per Inputs), DECISIONS rows, any prior blueprint (re-run: carry `## Your notes` verbatim). Never `draft.md`.
...
- **Downstream:** [[WORKFLOWS/episode-harden]] (S1.75 — reads run 1, writes the ratified `shape.md` run 2 reconciles against; run 1's hand-backs seed its agenda) → [[WORKFLOWS/brainstorm]] + [[WORKFLOWS/dev-capture]] (S2 — the aesthetic layer over the hardened spine) → [[WORKFLOWS/episode-feedback]] (S3 — **then this workflow regenerates, mandatory in v4**) → the drafting engine → [[WORKFLOWS/episode-runway]] Pass 3 (which measures the *draft* against §3b; this tool measured the *plan*).
