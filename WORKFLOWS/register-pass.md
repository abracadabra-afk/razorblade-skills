...
**The paragraph count — the other half of the same gate (added 2026-09-07; fork ruled 2026-09-06, a clause per workflow — `^backlog-wholefile-diff-gate` + `^backlog-protected-span-write-gate` (a); copied from [[WORKFLOWS/prose-expansion]] § Commit exit gate).** The span accounting above landed alone in 2026-08-10; the count that belongs beside it did not. When the revised passage is written, **diff the output against the working text paragraph-by-paragraph and report the count: N¶ in, N¶ out, exactly K differing, where K is the number of ruled changes** — the register-earned edits, plus the verdict sheet's RULED items in execute-only mode. Any hunk that is not a ruled change is a defect — revert it, do not rationalize it. Verify through the file tools, never a bash/mount read alone (DIR-005). **Scoped to this pass:** the source of the diff is the working text `resolve` picked (`draft.md` or the slate `clean-draft.md`), not the slate whenever the two differ; the count reports in the editorial note beside the `drift:` lines; and **sweep mode has no passage to diff**, so it states `0¶ out — no rev earned` rather than a count. This is an external check on what changed, which is the whole point — an internal promise about what was intended cannot catch an edit that is not of the kind the promise covers.
