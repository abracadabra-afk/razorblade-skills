...
## What changed (2026-07-26): a fourth route — check-in

The fork now has **four** destinations. A spoken **`daily check-in`** / **`weekly check-in`** / **`check-in.`** marker at or near the head routes to `_DICTATION INBOX/_checkin/`, and Stage B lands the body **verbatim** as one entry in `LIFE/CHECK-INS/entries/` before deriving any actionable from it ([[WORKFLOWS/daily-check-in]]).

Three things make this route different from the other three:

- **It is checked FIRST — ahead of dev and fiction.** A check-in is routinely canon-dense ("the register pass on Witchwood CH4 fought me all afternoon, the Dark Vale scene wouldn't land"), so canon density would otherwise classify CRE's diary as prose and stage it as a chapter draft. Checking it first is only safe because the marker set is narrow and multi-word — the `_DEV_MARK` discipline. The bare form (`check-in.`) additionally carries a **negative lookahead** on `on|with|about|at|to|regarding`, so *"let's do a check in on the cover designer"* stays an INBOX item.
- **No canon reconcile, no cleanup, ever.** A check-in isn't prose; a garbled project name in a diary entry costs nothing worth a false `[AUTHOR:]` flag. More importantly the entry is a **sample of how CRE was thinking that day**, and the weekly pass diffs those samples across weeks — a cleaned entry is a destroyed sample. Verbatim is the contract (and the safest way to honor DIR-015: text that is never touched is never "worked").
- **The body is kept WHOLE.** The runner does not carve tasks out of the transcript. Actionables are derived downstream, from the landed entry — carve it up here and there's nothing left to derive from.

Cadence (`daily` vs `weekly`) is detected from the marker and carried as `kind:` in the staging note; the weekly pass reads `kind: weekly` plus the daily entries in its window. A filename stem containing `check-in` also routes (the `_DEV_STEM` fallback pattern), so an STT-garbled marker still lands correctly.

Selftest coverage: 12 check-in cases + 4 false-fire guards (ordinary "checked in at the motel" prose, a fiction directive with a mid-clause check-in mention, an explicit task *about* a check-in, and the bare-marker-mid-clause case), with zero route changes across the 27 pre-existing cases. `runner.py --selftest-classify`.
...
- **Confidence tiers (flag tier RETIRED 2026-07-27):** exact alias -> auto at 1.0; fuzzy >=0.92 -> auto-correct; **short (≤1-word) targets need ≥0.95 to auto**; **below the auto bar -> left VERBATIM — the inline `[AUTHOR:]` flag tier is retired** (CRE-ruled 2026-07-27, `^backlog-dictation-runner-flag-tier`: the 0.86–0.90 band went **0-for-6** on live prose, `^obs-203`, while dictation-cleanup's context-aware flags went **15-for-15** on the same chapter — unrepaired garbles now fall through to cleanup's flags, the single flag surface). An all-common heard span (everyday words + stopwords — `which`, `the dark`, `moon was`) is still **suppressed outright** by the `COMMON_EN`/`STOPWORDS` guard before it can AUTO (^obs-119, see § below).
...
2. **Stage B — context triage pass (task Step 4c) — RETIRED 2026-07-27 with the flag tier.** Its input class no longer exists: the reconciler emits no `[AUTHOR:]` flags at all (auto-only; sub-auto matches stay verbatim, CRE-ruled — `^backlog-dictation-runner-flag-tier`, `^obs-203`). The `[AUTHOR:]` flags that remain in a cleaned note are **dictation-cleanup's own** — context-aware author calls (15-for-15 useful on the ^obs-203 chapter) that go to CRE's desk **untouched**; triaging those away would delete the useful surface the retirement ruling relies on. Kept here as history: the pass walked reconciler residue (`dark cavern → Dark Vale?`) downgrade-only with a `## Triage` log.

Net (post-retirement): Stage A's `COMMON_EN`/`STOPWORDS` guard still blocks all-common spans from silently AUTO-applying; everything below the auto bar stays verbatim; garble-flagging is dictation-cleanup's job alone, and its flags reach CRE unfiltered. `lexicon.py --selftest-common` + `--selftest-reconcile` cover the guard and the retirement contract.
...
> 0. **EMPTY-POLL GATE — the cheap check comes BEFORE the expensive setup (`^obs-166`).** Most polls find nothing, and staging + compiling `runner.py` costs real tokens, so never do it until work is confirmed. Make exactly ONE bash call: `D="<vault>/_DICTATION INBOX"; DROPS=$(find "$D" -maxdepth 1 -type f ! -name '.*' 2>/dev/null | wc -l); PEND=$(find "$D/_reconciled" "$D/_dev" "$D/_inbox" "$D/_checkin" -maxdepth 1 -type f -name '*.md' 2>/dev/null | wc -l); echo "drops=$DROPS pending=$PEND"`. *(`_checkin` added 2026-07-26 with the fourth route — a queue missing from this gate is a queue that never drains, the `^obs-166` stranding bug.)* The queue dirs may not exist yet — the `2>/dev/null` is deliberate (a missing dir counts as 0); a *pending* note is any `.md` sitting **directly** in a queue dir (finished ones have been moved into that queue's `done/`). Then branch: **`drops=0 && pending=0`** → STOP NOW, emit one line ("nothing to do"), end the run — no staging, no deps, no `_CHANGELOG` entry, no message (this is the normal case and it must cost one bash call and nothing else). **`drops=0 && pending>0`** → no new audio, only queued notes to drain: **skip steps 1–2 entirely** (running `runner.py` here would report `processed: 0` and, under step 2's stop rule, strand the queue) and go straight to steps 3/4/4b. **`drops>0`** → new clips; continue to step 1 (the full path).
...
> 4d. **Check-in branch (verbatim, land before deriving)** - for each new `_DICTATION INBOX/_checkin/*.md` not yet in `_checkin/done/`: read its frontmatter (`kind`, `confidence`) and its `## Body (verbatim transcript)` section. Write the body **VERBATIM** to `LIFE/CHECK-INS/entries/<date>-<kind>.md` using the entry template in [[LIFE/CHECK-INS/_CHECK-INS]] (carry the staging frontmatter's serialized YAML posture — DIR-004; suffix `-2` if that date+kind already exists, NEVER overwrite). **No cleanup, no reconcile, no summarizing, no tone repair** — the entry is a sample of how CRE was thinking that day and a tidied sample is a destroyed one. **Land the entry FIRST**, then derive actionables from it per [[WORKFLOWS/daily-check-in]] Step 2 (task-shaped → `TASKS/TASKS.md` schema-tagged; anything else that wants a home → `INBOX` verbatim with a `<!-- check-in <date> · intent: … -->` hint; reflection/mood/assessment → **stays in the entry only**). Ambiguous → one line in the entry's `## Open questions` as visible prose, never an HTML comment (DIR-012 clause 4). **Unattended runs never name a pattern, never promote to `_OBSERVATIONS`/`_BACKLOG`, and never reply with encouragement** (DIR-015 clauses 1 + 5). Move the staging note to `_checkin/done/`.
> 4c. **Flag-triage — RETIRED 2026-07-27 (CRE-ruled, `^backlog-dictation-runner-flag-tier`).** The reconciler no longer emits `[AUTHOR:]` flags (auto-only; sub-auto matches stay verbatim), so this pass's input class no longer exists. **Do NOT triage, downgrade, or remove the `[AUTHOR:]` flags that `dictation-cleanup` itself writes** — those are context-aware author calls (15-for-15 useful on the `^obs-203` chapter) and go to CRE's desk untouched. Carry them through exactly as cleanup wrote them. See § Flag-noise control.
...
> 6. Leave **ALL** `[AUTHOR:]` flags in place — they are `dictation-cleanup`'s context-aware author calls (Step 4c is retired; the reconciler emits none) - CRE rules them at the desk. Never bind a clip to a chapter (that is a desk action).
...
**Flag-triage (added 2026-06-26) — RETIRED 2026-07-27 with the reconciler's flag tier.** Step 4c existed to clear reconciler funcword false positives; the reconciler now emits no flags at all (auto-only, CRE-ruled — `^backlog-dictation-runner-flag-tier`). The `[AUTHOR:]` flags in a cleaned note are dictation-cleanup's own and are carried to CRE untouched. The deterministic Stage-A `COMMON_EN`/`STOPWORDS` guard remains (it blocks silent AUTOs) — see § Flag-noise control. Inbox stays verbatim and is exempt.
...
- Multi-word mishears whose phonetics diverge are caught by **seeded `_LEXICON.md` exact aliases** (separator-/case-insensitive, +0 noise — the DIR-014 measured winner) or, unrepaired, by dictation-cleanup's context flags; the reconciler's own flag tier is retired (2026-07-27, `^backlog-dictation-runner-flag-tier`). **Funcword over-fire** is handled by the Stage-A `COMMON_EN` guard (^obs-119), which now guards the AUTO band only.
...
- `_DICTATION INBOX/` - drop zone, accepts audio + `.txt`/`.md` transcripts (`README.md`, `_reconciled/` + `done/`, `_dev/` + `done/`, `_inbox/` + `done/`, `_checkin/` + `done/`, `_drafts/`, `processed/`).
- `LIFE/CHECK-INS/entries/` - check-in destination: Stage B writes the **verbatim** entry here, then derives actionables from it ([[WORKFLOWS/daily-check-in]]).
