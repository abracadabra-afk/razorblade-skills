...
> 2. **Integrity-gate the runner, then run it (`^obs-103` stale-mount guard).** Before executing, guard against a stale-mount truncation of the *script itself*: the bash mount can serve a TRUNCATED partial of `runner.py`/`lexicon.py` while Dropbox is mid-sync (the `^obs-073`/`^obs-095` hazard — but a truncated *script* is wrong **behavior**, not just wrong data, so it must be caught before `python3` runs). Verify the mounted `runner.py` (and `lexicon.py`) against the authoritative **file-tools** copy — compare line count and/or run `python3 -m py_compile <file>`; on a line-count mismatch or a compile/syntax error, refresh the workflow-dir copy from the file-tools read (write the complete file back into the workflow dir) and run **that**. Only then run `python3 "<vault>/WORKFLOWS/dictation-runner/runner.py"`. It prints JSON of what it processed; each result carries a `route` (`fiction` or `inbox`). If `processed: 0`, stop - nothing to do, no log entry.
