...
3. **Monday's day-launch closes the loop:** the standing **`every:mon` "Confirm the week" task in `TASKS.md`** (`#p2 win:ops #quick` — *was `#p1` "Ratify the week"; demoted and re-scoped 2026-08-19 by v1.2, since under clause 2a nothing downstream starves when it is skipped*) surfaces on Monday's board through the normal recurrence machinery, so it reaches CRE's board and phone even with every cadence running unattended; day-launch's v2.9 staleness escalation is the backstop (same item, deduped). Do not argue its first-class status from the old `#p1` band — `TASKS.md` carries the live name and priority, and this doc is the side that went stale (caught by `task-audit` 2026-08-23, fixed 2026-08-24). **The ratify leg runs Step 4a (state diff) BEFORE presenting** — the conservatism that makes this mode safe (propose, seed nothing) is the same thing that opens the staleness window, so the attended half is the only actor positioned to close it (`^obs-248`). On CRE's go — attended, one pass — the seeds land in Active per Step 5 and the block drops its PROPOSED stamp.
...
5. **Expire `not now` flags as part of composing the week (v1.2, 2026-08-19; absorbed into this doc 2026-08-24).** A `not now` flag CRE set last week — `<!-- flag: not now, YYYY-MM-DD -->` in `TASKS.md`, per [[TASKS/TASK-SCHEMA]] (:98, :125) — is a **ONE-WEEK deferral, not a pause**: clear it when the new week is composed. A CRE-ruled *pause* is indefinite and stays untouched. This clause was ruled canon in `TASK-SCHEMA` and carried by the `week-shape-runner` prompt from 2026-08-19, but never landed here — so this doc was the stale side, and an agent reconciling on "the doc wins" would have **dropped a ruled behaviour**. Caught by `task-audit` 2026-08-23 (item 4), fixed 2026-08-24. DIR-014: a ruling is not landed until it reaches the surface that binds the tool.
...
<!-- v1.2a 2026-08-24 (CRE-ruled, Monday gate-bin pass) — doc-side half of task-audit 2026-08-23 item 4, which found this doc was the STALE side on two clauses the prompt carried correctly. (a) Unattended cl. 3's ratify hand-off still read `#p1` "Ratify the week" and argued first-class status from that band; the live item is `#p2` "Confirm the week" (TASKS.md:46, re-scoped 2026-08-19 by v1.2 cl. 2a). (b) The `not now` one-week expiry — canon in TASK-SCHEMA:98/:125 since 2026-08-19 and live in the week-shape-runner prompt since the same day — had never landed here, so an agent reconciling on "the doc wins" would have dropped it; added as unattended cl. 5. Both are DIR-014 shapes: the ruling reached the tool-binding surface and not the canon doc. Prompt-side gaps from the same audit item fixed in the runner prompt the same session (DIR-016). -->
