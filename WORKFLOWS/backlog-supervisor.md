...
- **## References** — every file the work needs, by full path, each with one line on what it carries and why it is here. Anything the agent would otherwise have to go find. Include the directives that bind this particular job, by number. **Name the substrate a reference needs when it is not the file tools** — `C:\Users\Chad\Claude\Scheduled\` is unreachable by `Read` and needs the host route, a fact the pilot plan omitted and the agent had to discover. **A `_BACKLOG` item is addressed by its anchor** (`^backlog-…`), reached by pathed `Grep -n` then offset `Read` (§ Candidate source); a line number may be given only as a hint stamped with the date it was read (*"line 313 as read 2026-09-08"*), never as the address — two consecutive agent fires (2026-09-09, 2026-09-10 receipts in `SYSTEM/reports/backlog-agent-runs.md`) found the number stale by 10–14 lines within two days.
...
- **## Completion conditions** — objective, checkable statements. *"`SYSTEM/reports/2026-09-04-approval-gated-openers.md` exists and lists every one of the 14 registered task prompts with a flag or a clear."* Not *"the sweep is done."* These are what the audit reads; a vague condition makes the audit a judgment call, which is the one thing this loop cannot afford. **Check the conditions against this plan's own Defer section before serving** — a condition that forbids what the Defer rule authorizes is a self-contradicting plan, and the agent will be right whichever way it goes. **A `last_updated:` condition on a canon doc reads *"bumped to the date of the run that edits it"* — never a literal date, never regressive:** a literal target goes stale the moment another fire touches the same doc (the 2026-09-11 fire found `2026-09-09` demanded against a live `2026-09-10`, receipt in `SYSTEM/reports/backlog-agent-runs.md`). A plan targeting `skills-src/` files carries **no** `last_updated:` condition at all while the 2026-09-10 rulings line on packable frontmatter is unruled.
