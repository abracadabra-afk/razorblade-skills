...
- **Scope of the preflight in this doc (per-doc scoping, DIR-019 leg (b) shape).** An **`ingested` chapter has no revision chain by design** — Step 2 N/A-notes `revisions/README.md`, so an empty or N/A-stubbed `revisions/` here is the expected state and the shared text's last sentence is the whole of what applies: note it in one line and proceed. The preflight fires only on the **re-ingest** path, where a `draft.md` already exists and is about to be overwritten; it never changes Step 2's *"never overwrite an existing `CHAPTER <N>/` (halt if present)"* rule, and it never changes this step's own next-chapter selection — a chapter whose folder is not yet built has nothing to account.
