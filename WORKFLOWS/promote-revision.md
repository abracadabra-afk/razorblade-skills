...
**Scope of the gate in this doc (per-doc scoping, DIR-019 leg (b) shape).** It covers the **body** only. The frontmatter rewrite in the mapping above is this pass's job, not a diff hunk — never revert a mapped field to satisfy the count. It does not reach Step 3b's `superseded_by:` stamps on *other* files (supersession accounting, not a prose commit), and it does not replace the mapping's existing **read** of the rev note's `protected_spans_touched:` for un-updated witnesses — that check reads the upstream pass's accounting before promoting; this one writes this pass's own after. **Hand-landing mode ("account the landing"):** there is no promoted body to diff, so the paragraph count applies instead to Step 3b item 4's archive — `revisions/<date> - draft N superseded.md` must be byte-exact against the `draft.md` it preserves — and CRE's landing is itself the ruling on any span it moved: record the accounting, never re-open it as a question (§3).
