...
7. **A `binding_surface: true` threads.md gets the binding-surface guards (dec-030, ratified 2026-08-26; first adopter — Witchwood).** Under the layered-authority principle (*dev capture founds · landed canon supersedes · debts are invariant · beats are directional · containers and transit are authorial*), a threads.md stamped `binding_surface: true` is the project's **sole binding surface** — every other forward surface (the `DEV/` fan, beat material, runway, `brief.md`) is advisory. That removes the old redundancy where the dev layer was a second, informal memory of intended payoffs, making threads.md a single point of failure maintained by this derive pass — in a vault with the DIR-004/DIR-005 truncation history. Two guards ride that status (dec-030 condition 2; mirrored from the threads.md banner — this doc is the tool-binding surface per DIR-014):
   - **Every thread sync states what it did NOT check** (DIR-018) — see Step 6. No bare clean signal on a binding surface.
   - **Write-time debt accounting** (DIR-010's authored-artifact corollary) — the Step 3 diff accounts for **every** CH-tagged thread event of the synced chapter as `kept` / `reworded → new span` / `dropped`; a drop gates at Step 4, never silently retires. Post-hoc span matching cannot recover this (DIR-014's matcher clause) — the accounting happens at sync time, while the diff is in hand.
   - Corollary of "containers and transit are authorial": a thread entry records what a payoff **depends on** (a `Dependency` line), never where it sits — do not write sequence/chapter seats into threads.md.
...
- `REFERENCE/threads.md` — add planted threads with provenance; append advances to open threads; move paid/abandoned threads to **Settled** (History line, never deleted); fill the **Blind-read pickup** line per planted thread. Author-note lines are never overwritten. **On a `binding_surface: true` threads.md (principle 7):** every CH-tagged thread event of this chapter carries its Step-3 accounting (`kept` / `reworded → new span` / `dropped, ruled <date>`); an unaccounted drop is a defect — halt and gate, don't rationalize. Never write sequence/chapter seats; dependency lines only.
