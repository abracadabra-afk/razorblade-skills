---
name: dev-capture
description: Capture and organize a fiction project's molten DEVELOPMENT talk — the upstream-of-the-brief "what is this about / what do I love / what has to happen here" thinking — into the project's DEV/ tree, WITHOUT drafting prose. Use whenever CRE asks to "capture the dev," "develop this," "route the dev intake," "sort the dev transcript," "file the development," or "run the dev router," and hands over a cleaned dictation transcript aimed at a project's DEV/ layer. It segments the transcript, routes each segment by spoken cue (else by inferred content, else holds), sharpens scene entries in place (sculptor — raw transcript to the audit floor, one-line pointer left behind), measures scenes against the _DEV.md taste anchor, silently logs contradictions with banked manuscript to _LEDGER.md, and notices process patterns in _POETICS.md. Do NOT use it to draft prose from dictation (that is dictation-transcoder, "slate this dictation"), to build a forensic outline from a filled brief (that is runway-builder), or to copy-edit a transcript (that is dictation-cleanup). It sits BELOW the brief and never promotes anything into REFERENCE canon (that crossing is separate and gated). Requires the project's DEV/ tree to exist — if it doesn't, run the bundled scaffolder first.
---
