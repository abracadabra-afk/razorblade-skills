...
| `protected_spans_touched` | **write it** — this pass's own accounting, per the commit exit gate below. Distinct from the rev note's `protected_spans_touched:` that Step 2 reads: that one is the upstream pass's record, consulted before promoting; this one is what *this* commit did. |
...
**Commit exit gate — diff against source (added 2026-09-07; fork ruled 2026-09-06, a clause per workflow — `^backlog-protected-span-write-gate` (a) + `^backlog-wholefile-diff-gate`; copied from [[WORKFLOWS/prose-expansion]] § Commit exit gate).** When the promoted body is written, **diff the output against the source paragraph-by-paragraph and report the count: N¶ in, N¶ out, exactly K differing, where K is the number of ruled changes.** On this pass **K is 0** — promotion copies the revision's prose byte-for-byte and never changes a word — so *any* differing hunk is a defect: revert it, do not rationalize it. Verify through the file tools, never a bash/mount read alone (DIR-005).

**And account for the protected spans.** The promoted `draft.md`'s frontmatter carries `protected_spans_touched:` — every span from `REFERENCE/protected-patterns.md` plus the carried-forward `protected_patterns` that this commit touched, each `kept` / `reworded → "<new span>"` (witness updated same session) / `dropped — ruled by CRE <date>`; `[]` stated when none, and on a clean promotion `[]` or all-`kept` is the expected reading. Serialized, never hand-formatted (DIR-004). Same logic as the paragraph count, extended from paragraphs to protected spans: the edit moment is the only place "reworded" and "violated" are distinguishable.

**Scope of the gate in this skill (per-doc scoping, DIR-019 leg (b) shape).** It covers the **body** only. The frontmatter rewrite in the mapping table above is this pass's job, not a diff hunk — never revert a mapped field to satisfy the count. It does not reach Step 3b's `superseded_by:` stamps on *other* files (supersession accounting, not a prose commit), and it does not replace Step 2's existing **read** of the rev note's `protected_spans_touched:` for un-updated witnesses — that check reads the upstream pass's accounting before promoting; this one writes this pass's own after. **Hand-landing mode ("account the landing"):** there is no promoted body to diff, so the paragraph count applies instead to Step 3b item 4's archive — `revisions/<date> - draft N superseded.md` must be byte-exact against the `draft.md` it preserves — and CRE's landing is itself the ruling on any span it moved: record the accounting, never re-open it as a question (DIR-019 §3).
