...
Resolve the chapter folder (ask if not named). Determine the **working text** — `draft.md` if present, else the newest `slate/…clean-draft.md`, else the raw `dictation/` — and note how rough it is (a dictation-stage workshop is about intent and shape, not anything line-level).

**Author-landing preflight (DIR-019 §3, `^backlog-author-landing-preflight`).** Before anything else in this step, diff `draft.md`'s body against the newest entry in `revisions/`. Match — or `draft.md`'s `source_revision` names that entry — → proceed. Mismatch, or `revisions/` holds nothing matching the live body → **CRE hand-landed this draft**: run [[WORKFLOWS/promote-revision]] Step 3b in hand-landing mode first (archive the superseded body byte-exact to `revisions/<date> - draft N superseded.md`, then stamp the folder's stale derives, retire the moot rulings, rewrite `draft.md`'s own open-items list), and only then continue. Never ask him to confirm the landing — his landing is the ruling. Safe op, logged, never gated. No `revisions/` folder at all (author-direct route) → note it and proceed; there is nothing to diff against. — the workshop is read-only on everything but `workshop.md`, and the preflight is the one write it is allowed to trigger, because it is `promote-revision`'s write and not the workshop's.

**Scope of the preflight in this doc (per-doc scoping, DIR-019 leg (b) shape).** Principle 4 is **not relaxed**: this skill's own write surface is still `workshop.md` plus the session logs, and nothing here licenses it to touch `draft.md`, `slate/`, `revisions/`, `REFERENCE/` or StoryLine on its own account. What the preflight does is **hand off** — it detects the unaccounted hand-landing and runs [[WORKFLOWS/promote-revision]] Step 3b, whose writes are that workflow's, made under that workflow's own scope lock, before the context pack is loaded. Two consequences for the rest of this doc. **Step 6's unchanged-check is measured from the post-preflight state**, not from the state the session opened in: a Step 3b archive, a `superseded_by:` stamp, or a rewritten `draft.md` open-items list is the preflight's accounting and is reported as such, not surfaced as the bug that check hunts — anything changing *beyond* that is still a bug. And **the preflight adds no halt and supplies no working text**: this doc states no stop-conditions halt of its own — principle 8 governs a thin or missing working text exactly as it did, and the preflight neither supplies one nor makes a rough one sufficient. A chapter with no `draft.md` yet — the ordinary pre-dictation workshop — has nothing to diff: note it and load the pack. **Principle 2 is untouched** — the preflight moves no words and writes no fiction; it is bookkeeping in front of the read, never a gate in front of the conversation.
