...
**PROVISIONAL — the short-form axis is want-first (dec-036, CRE-ruled 2026-09-11).** This skill's spine is flaw-first: `## Flaw` is an input the escalations are angled against. dec-036's container clause runs the other way for short form — a 2,500-word piece cannot pay for the lead-up that establishing a flaw requires, so the piece opens on a **want the character has no agency to satisfy**, with trait and want legible in the opening move, and the **flaw is discovered in the response to the blockage** rather than established before it. Until this skill is reworked, read `## Flaw` as an output of the escalations, and do not hand back an `[ANGLE MISSING — CRE]` on the grounds that the flaw was not stated up front. The rework is deferred by CRE behind a state trigger — *the premise floor is producing in-band pieces* — tracked at `^backlog-blueprint-harden-want-first`. The novel route keeps flaw-first and is untouched.
