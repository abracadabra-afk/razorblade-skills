...
**Author-landing preflight (DIR-019 §3, `^backlog-author-landing-preflight`).** Before anything else in this step, diff `draft.md`'s body against the newest entry in `revisions/`. Match — or `draft.md`'s `source_revision` names that entry — → proceed. Mismatch, or `revisions/` holds nothing matching the live body → **CRE hand-landed this draft**: run [[WORKFLOWS/promote-revision]] Step 3b in hand-landing mode first (archive the superseded body byte-exact to `revisions/<date> - draft N superseded.md`, then stamp the folder's stale derives, retire the moot rulings, rewrite `draft.md`'s own open-items list), and only then continue. Never ask him to confirm the landing — his landing is the ruling. Safe op, logged, never gated. No `revisions/` folder at all (author-direct route) → note it and proceed; there is nothing to diff against. — `resolve` already reports both the working text and `revisions/`; the preflight is one comparison on data the script has already printed.

**Scope of the preflight in this doc (per-doc scoping, DIR-019 leg (b) shape).** It runs on the **`draft.md` case only** — where `resolve` picked the newest slate `clean-draft.md` because `draft.md` carries no real content, there is no landed body to have been hand-landed: say so in one line and proceed. It is **additive to the supersession triage below, never a replacement**: the triage retires rulings against whatever the working text now is, the preflight is what makes sure the body they were retired against was preserved. And it does not reach the Step 3 commit exit gate's paragraph count — that is an exit check on what *this pass* changed; this is an entry check on what CRE changed before the pass ran.
...
**The paragraph count — the other half of the same gate (added 2026-09-07; fork ruled 2026-09-06, a clause per workflow — `^backlog-wholefile-diff-gate` + `^backlog-protected-span-write-gate` (a); copied from [[WORKFLOWS/prose-expansion]] § Commit exit gate).** The `protected_spans_touched` accounting above landed alone in 2026-08-10; the count that belongs beside it did not. When the revised passage is written, **diff the output against the working text paragraph-by-paragraph and report the count: N¶ in, N¶ out, exactly K differing, where K is the number of ruled changes** — the register-earned edits, plus the verdict sheet's RULED items in execute-only mode. Any hunk that is not a ruled change is a defect — revert it, do not rationalize it. Verify through the file tools, never a bash/mount read alone (DIR-005). **Scoped to this pass:** the source of the diff is the working text Step 1 resolved (`draft.md` or the slate `clean-draft.md`), not the slate whenever the two differ; the count reports in the note beside the `drift:` lines; and **sweep mode has no passage to diff**, so it states `0¶ out — no rev earned` rather than a count. This is an external check on what changed, which is the whole point — an internal promise about what was intended cannot catch an edit that is not of the kind the promise covers.
