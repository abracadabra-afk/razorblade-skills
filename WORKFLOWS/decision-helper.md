...
**Phrasing rule for platform actions (`^obs-196`, 2026-07-19).** When an execution note or task line tells CRE to **operate a control on a third-party platform** (a toggle, channel, setting, opt-out), write it as a **confirmation, not a command**: *"confirm whether X can be turned off; if the control doesn't exist, record why that's acceptable"* — never *"turn off X."* A receipt sourced to a vendor's docs or a community/advice post describes what that UI looked like **when the post was written**; it is a dated claim, not a probe (DIR-010). Source instance: dec-016's "turn off Ingram's Amazon channel" — standard dual-print advice repeated across ALLi + KDP Community — named a control IngramSpark does not expose on standard accounts, and the resulting task carried three days against a phantom switch. Command phrasing also **hides the real condition**: the actual exposure was the same-ISBN product-match requirement, which the toggle framing obscured entirely. Where the note names a control, name **where it lives** too, so the first execution session knows what it's looking for.
