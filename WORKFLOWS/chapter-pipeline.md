...
- **Block canon scratch.** Between dictations within a sequence, the AI auto-derives a lightweight running ledger of emergent canon from each raw dictation (objects named on the fly, decisions, new phrases) — not a full `canon-sync`, just a scratch the next dictation leans on. Bounded because a sequence carries one threshold's worth of canon. Kills in-block drift at near-zero creative cost. **Stored at `<project>/SEQUENCES/SEQUENCE <N> - <NAME>/canon-scratch.md`** (append-only, one dated block per chapter); it is *provisional* — `canon-sync` reconciles it into `REFERENCE/` at the sequence boundary (Phase 5), after which the scratch is archived, not trusted as canon.
...
14. **land-chapter** ([[WORKFLOWS/land-chapter]]) — the register-promote is already done, so this runs `canon-sync` → `storyline-sync` to bring REFERENCE current and regenerate the StoryLine mirror. At the **sequence boundary**, this is where the held-in-head/block-scratch canon is formally reconciled before the next sequence is planned.
