...
SEQUENCE LOOP  (the batch unit — one transformation threshold, opening/middle/closing chapters)
  └─ Phase 1  Construct      Workshop-1 (intent + envelopes; consumes the descent's chapter map) → runway-builder   [whole sequence, one sitting]
  └─ Phase 2  Draft          dictate → dictation-cleanup                           [whole sequence, peak hours]
  CHAPTER LOOP  (per chapter, the convergent work — off-peak)
     └─ Phase 3  Develop     Workshop-2 (warm read, rule) → blind-response (execute) → loop-clearer → structure-promote
...
2. **~~Blind-read becomes a subagent feeding Workshop-2~~ — SUPERSEDED 2026-08-03 (QA-tier ruling, [[WORKFLOWS/pipeline]] § QA tiers).** Reader diagnostics scope to the **reader unit**: a per-chapter cold read of a mid-book chapter is structurally artificial (a real reader arrives carrying every prior chapter — the cold read over-flags done planting and cannot see book-scale defects), and it taxes the drafting loop's momentum. Phase 3 now runs **Workshop-2's warm, spec-aware read alone**; `blind-response` keeps its executor role, its input now Workshop-2's ruled structural fixes (which keeps Workshop read-only, its design invariant). Blind reads run at the part boundary and in the manuscript battery (tiers 3–4), plus an optional **pull-only canary** on a sequence's opening chapter when CRE calls it.
...
- **Scaffold exactly one sequence ahead — no more.** Workshop-1 constructs the whole sequence (all its chapters) in one sitting; that is the batch. Do not construct the *next* sequence until this one has landed: sequence-type selection depends on "current character psychological state," and crossing this sequence's threshold *changes* that state, so planning ahead plans against a stale starting point. (Same staleness guard as [[WORKFLOWS/shape-the-part]]'s one-part-per-run rule — the two rules are the one principle at two altitudes.) **Feed-forward belongs between sequences (formal canon-sync); held-in-head canon belongs within one (bounded, cheap).**
...
- **Chapter-one canary (pull-only since the 2026-08-03 QA-tier ruling).** Available on CRE's word, never scheduled: a clean-room cold read of the sequence's **opening** chapter. The rationale stands — the opening establishes the transformation context, and an arc-level problem there is the one thing not to discover after the middle and close are dictated — but it is now an instrument CRE reaches for when an opening feels risky, not a standing leg. Line-level findings defer happily.
...
1. **Workshop-1** ([[WORKFLOWS/workshop-chapter]], plan mode) — **where the project runs the layer battery + [[WORKFLOWS/shape-the-part]] descent (Witchwood), the plan already exists**: the part's CRE-ruled **chapter map** + `DEV/scenes/` entries are battery-locked structure, and Workshop-1 **confirms intent against them — it never re-plans a locked layer** (a structural surprise here routes to author-flags/dev-capture, not a re-plan). It authors the **sequence envelope** (POV frame, transformation threshold entry→exit with sign, register band, escalation range, entry/exit canon), scaffolds each mapped chapter ([[WORKFLOWS/chapter-init]] — whose Step 4b seeds each `brief.md` from the chapter's mapped scene entries: the brief-fill seam) and **derives each chapter `envelope.md`** from the sequence envelope. CRE confirms the briefs (S2). On projects without a DEV descent, Workshop-1 plans the sequence itself (the original mode). Output: briefs + envelopes for the whole sequence.
2. **runway-builder** ([[WORKFLOWS/runway-builder]]) — `brief.md` + the chapter envelope → `runway.md` for each chapter, in the **forensic logic-beat form (GOAL→BUT→THEREFORE)** — the leg doc is canonical for the form; the beat-envelope/temperature description this step originally carried was superseded 2026-06-19 (`^obs-108`: temperature tags read as riddles at the mic; structure only, CRE supplies the skin live). This is what CRE dictates from. Then **derive the initial working canon** (`canon-sync` working mode) from the skeletons, so forward chapters have context before a word is dictated.
...
**Energy receipts (CRE-ratified 2026-07-15):** each dictated chapter's changelog entry carries one instrumentation line — `energy: takes/retakes <n> · open-loops raised <n> · revision cycles <n>` — so the shape-before-dictate route's core claim (energy saved until the most effective part) accrues receipts against the CH5–7 baseline (`^obs-104`) instead of running on feel. One line at a log step that already exists; no new process.
...
3. **Reader diagnostics move to the reader-unit tier — 2026-08-03.** Per-chapter blind-read and blind-response *triage* removed from the inner loop (wrong unit + momentum tax); part-boundary blind read recommended, full reader battery at manuscript complete, canary pull-only. Canon home: [[WORKFLOWS/pipeline]] § QA tiers.
