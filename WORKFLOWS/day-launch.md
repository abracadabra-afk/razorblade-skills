...
## Weekend mode — derive-only (v2.4, CRE-ruled 2026-07-19)

**On Saturday and Sunday, day-launch does not propose. It only records.**

Why: CRE works hard Mon–Fri, and *"if weekends feel no different it is an endless slog."* His actual weekend pattern is catchup, personal, and spur-of-the-moment work — self-directed and often productive (2026-07-19 produced a dictated + drafted Godsrift chapter). Proposing a weekday-shaped board into that mode does not direct the day; it manufactures a failing score for a day that had no target. The receipt line had accumulated `0/0 · 0/5 · 0/4 · 0/5` across weekend days — **a losing streak counter, assembled by a skill whose design rule is "no streaks, no guilt."** The fix is not a smaller weekend board: a board of two is still a board you can fail. It is the absence of a target.

Rules:

1. **No proposal, no first domino, no 3–5.** Skip Steps 2–4 entirely. `TODAY.md` is written with `status: weekend` and no item list.
2. **Time-bearing items still surface** — an appointment or a hard `due:` today — as a **single reminder line**, never a board. A `blocks:<who>` item ([[TASKS/TASK-SCHEMA]]) also surfaces, because someone else's time is attached and that does not respect the calendar.
3. **The receipt is descriptive and has NO denominator.** Run the Step-0 derive pass (it already reads `_CHANGELOG` + artifacts and is built exactly for off-board work), then write what happened: `2026-07-19 Sun — Godsrift B3-CH05 dictated + drafted, returned to Ashley (off-board)`. Never `X/Y`. **You cannot fail a day that had no target.** A weekend with no artifacts writes `— quiet day`, full stop, with no framing.
4. **Weekend days are excluded from every carry counter.** Monday's run computes "Nth-day carry" over **weekdays only**. The 2026-07-19 work-through pass found two items reading as 3- and 5-day stalls that were, stripped of weekend days, one item at two weekday misses and one that had never had a working day — a measurement artifact that read as an avoidance pattern and was very nearly diagnosed as one.
5. **Monday is normal.** No catch-up framing, no "you missed the weekend," no carrying weekend guilt into the week's first board.
...
1. **Append the receipts line — what happened LEADS, the score follows (v2.5, 2026-07-19).** Format: `YYYY-MM-DD — did: <what actually landed, planned and unplanned together> · planned X/Y · carried: <items>`. Count `- [x]` vs total as before (derived checks count), but the `X/Y` is **no longer the headline**. Rationale: on 2026-07-17 the line read `0/4` followed by the largest haul on the board — the least accurate number on the line sat in the position of prominence, and a string of them assembled into a losing streak inside a skill whose design rule is "no streaks, no guilt." **Planned and actual stay separate columns and are never merged** — completed off-board work is NEVER back-filled into the item list as `- [x]`, because that rewrites the plan to match the day and destroys the plan-vs-actual gap. That gap is the most diagnostic thing on the file: it is what exposed the missing `blocks:<who>` schema field on 2026-07-19, when a co-author chapter outranked every board item and appeared on none of them. Unified with weekend mode: **weekdays carry `did:` + `planned X/Y`; weekends carry `did:` alone, no denominator.**

   **Honest limit:** the derive pass only sees work that left an artifact in the vault or `_CHANGELOG`. Work done entirely outside it — a call, an email decision, a handoff arranged by text — leaves nothing to derive and no tuning fixes that. Those need a one-line drop into [[INBOX]] when they happen or they stay invisible.
...
<!-- v2.5 2026-07-19, CRE-ruled (same day as v2.4): RECEIPT ORDER INVERTED — close-out Step 1 now leads with `did:` (what actually landed, planned + unplanned together) and demotes the score to `planned X/Y`; template updated to match. Format change only, no behavior change. Rationale: the 07-17 line read `0/4` ahead of the largest unplanned haul on the board — the least accurate figure held the headline, and a run of them assembled a losing streak inside a skill whose design rule is "no streaks, no guilt" (same Pattern #11 defect v2.4 fixed for weekends, in its weekday form). Explicitly REJECTED at the same time: back-filling completed off-board work into the item list as `- [x]` — that rewrites the plan to match the day, makes the denominator meaningless, and destroys the plan-vs-actual gap, which is the file's most diagnostic signal (it is what exposed the missing `blocks:<who>` field on 07-19). Unified rule: weekdays `did:` + `planned X/Y`; weekends `did:` alone. Added the honest limit — the derive pass sees only what left an artifact; out-of-vault work needs an INBOX line. Repack BATCHED, not urgent (^backlog-repack-day-launch-v25). -->
<!-- v2.4 2026-07-19, CRE-ruled: WEEKEND MODE (derive-only) added as its own section — Sat/Sun skip Steps 2-4 entirely (no proposal, no first domino, no 3-5), run only the Step-0 derive pass, and write a DESCRIPTIVE receipt with no denominator; weekend days excluded from all carry counters; `blocks:<who>` + time-bearing items still surface as a single reminder line. Root cause: the weekend receipts had accumulated 0/0 · 0/5 · 0/4 · 0/5 — a losing streak counter inside a skill whose design rule is "no streaks, no guilt" — because the board scored non-working days as failed working days. Surfaced by the first live `work-through` pass (2026-07-19), which had itself very nearly diagnosed the resulting inflated carry-counts as a #3+#5 avoidance pattern; the same pass found a Godsrift chapter dictated and drafted entirely off-board (co-author handoff to Ashley), which the schema could not express — hence `blocks:<who>` in TASKS/TASK-SCHEMA. See DIR-015's Why clause + _CHANGELOG 2026-07-19. Skill source mirror + desktop repack PENDING (DIR-009 — the installed .skill is now behind this doc; weekend mode is not live until it is repacked, and the first Saturday it matters is 2026-07-25). -->
