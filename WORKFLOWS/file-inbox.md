...
```
pip install --break-system-packages odfpy openpyxl pdfplumber python-docx pyyaml
```
All pure-Python; no system binaries. `pyyaml` is required by the staging-note writer (DIR-004). Each extractor degrades gracefully: a missing lib (or a `.docx`/`.odt` that won't parse) falls back to a raw-XML strip and a flag in the note rather than crashing the run. **Upgrade path:** vendor the wheels offline (like the whisper model) to drop the per-run install; add image OCR (tesseract) if Stage-B vision proves insufficient.
