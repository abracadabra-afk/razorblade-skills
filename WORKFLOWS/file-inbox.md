...
## Dependencies (VENDORED as of 2026-07-10 — `^backlog-file-inbox-hardening` (a))

The extractor libs (`odfpy` / `openpyxl` / `pdfplumber` / `python-docx` / `pyyaml` + their transitive deps) are **vendored in the vault at `WORKFLOWS/file-inbox/.deps/`** (~74 MB, Linux x86_64 tree matching the sandbox — the whisper-model pattern: ship the artifact, drop the per-run pip install). `runner.py` self-locates them: it prepends `<VAULT>/WORKFLOWS/file-inbox/.deps` (then a staged-copy sibling `.deps`, then `/tmp/fi_libs` / `$FILEINBOX_DEPS` if present) to `sys.path` before the extractor imports, so a normal run needs **no install at all**. Verified 2026-07-10: selftest 6/6 + live smoke (xlsx/docx/pdf) extracting from the vendored tree.

**Fallback only** (if a run reports missing extractors — e.g. the vendored tree is damaged or the platform differs): `pip install --no-cache-dir --target /tmp/fi_libs odfpy openpyxl pdfplumber python-docx pyyaml` and re-run — a fixed root-fs `--target` dir, **never** `--break-system-packages` (lands in `~/.local` on the shared full device (`/dev/sdc`) → ENOSPC, `^obs-102`). `pyyaml` is required by the staging-note writer (DIR-004). Each extractor still degrades gracefully: a missing lib (or a `.docx`/`.odt` that won't parse) falls back to a raw-XML strip and a flag in the note rather than crashing the run. **Remaining upgrade path:** add image OCR (tesseract) if Stage-B vision proves insufficient (sub-item (b), gated).
...
- ~~**Per-run pip install** of the extractors.~~ **RESOLVED 2026-07-10:** extractors vendored at `WORKFLOWS/file-inbox/.deps/`; runner self-locates (`^backlog-file-inbox-hardening` (a)). pip remains as documented fallback only.
