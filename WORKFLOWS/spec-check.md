...
0. **Author-landing preflight (DIR-019 §3, `^backlog-author-landing-preflight`) — runs *outside* any pass context, before the battery starts.** Before anything else in this step, diff `draft.md`'s body against the newest entry in `revisions/`. Match — or `draft.md`'s `source_revision` names that entry — → proceed. Mismatch, or `revisions/` holds nothing matching the live body → **CRE hand-landed this draft**: run [[WORKFLOWS/promote-revision]] Step 3b in hand-landing mode first (archive the superseded body byte-exact to `revisions/<date> - draft N superseded.md`, then stamp the folder's stale derives, retire the moot rulings, rewrite `draft.md`'s own open-items list), and only then continue. Never ask him to confirm the landing — his landing is the ruling. Safe op, logged, never gated. No `revisions/` folder at all (author-direct route) → note it and proceed; there is nothing to diff against. — the battery's own DIR-019 triage (below) assumes the developmental pass moved the text; this covers the case where CRE moved it. Like `panel-read`, the check never enters the Pass 1 context: it is folder bookkeeping done before the battery starts. **The clean room is untouched by this** — item 0 is run by whoever invokes the battery, in the invoking context, and its result is never pasted into Pass 1's fresh thread; a reader who would have to open `revisions/` to run the check is the check in the wrong place.
