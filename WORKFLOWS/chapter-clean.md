...
**Author-landing preflight (DIR-019 §3, `^backlog-author-landing-preflight`).** Before anything else in this step, diff `draft.md`'s body against the newest entry in `revisions/`. Match — or `draft.md`'s `source_revision` names that entry — → proceed. Mismatch, or `revisions/` holds nothing matching the live body → **CRE hand-landed this draft**: run [[WORKFLOWS/promote-revision]] Step 3b in hand-landing mode first (archive the superseded body byte-exact to `revisions/<date> - draft N superseded.md`, then stamp the folder's stale derives, retire the moot rulings, rewrite `draft.md`'s own open-items list), and only then continue. Never ask him to confirm the landing — his landing is the ruling. Safe op, logged, never gated. No `revisions/` folder at all (author-direct route) → note it and proceed; there is nothing to diff against. — a chapter CRE hand-landed between sittings must be accounted before a new slate run is bound into the same folder.

**Scope of the preflight in this doc (per-doc scoping, DIR-019 leg (b) shape).** Its *"never ask him to confirm"* clause is about the **landing**, and it does not reach the binding rule above: the clip↔chapter match stays confirmed with CRE at trigger, never guessed, and the preflight's result is reported in the Gate A screen rather than substituted for that confirmation. It runs at the head of this step, **before** Leg 0 — so a hand-landing is accounted even when Leg 0 no-ops on an already-filled `envelope.md`, and it is not made moot by [[WORKFLOWS/dictation-preflight]] carrying the same clause one leg down (carried here as well, cheaply, so the orchestrator is safe on its own). It changes nothing about the gated-floor skip or the posture declaration in Step 0.
